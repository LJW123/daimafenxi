  ...
  if (map.getFreeCameraOptions) {
    ...
    const scale =
      pitchRadians < 0.001
        ...
        : (altitudePixels * Math.sin(pitchRadians)) /
          cameraToCenterDistanceGround;
    viewState.zoom = Math.log2(scale);

    const cameraZFromSurface =
      (altitudePixels * Math.cos(pitchRadians)) / scale;
