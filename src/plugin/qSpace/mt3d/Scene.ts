...
const MercatorCoordinate: any = QMapGl.MercatorCoordinate;
const Evented: any = QMapGl.Evented;
const LngLat: any = QMapGl.LngLat;
export type mapProps = QMapGl.Map & {
  transform: {
    maxPitch: number;
    _calcMatrices(): void;
...
export class Scene extends Convert {
  ...
  constructor(options: SceneOptions) {
    ...
    this.showFog = defined(options.showFog, false);
  ...
  update(matrix?: Float64Array) {
    this.updateSize();
    let list = this.scene.children.filter((it: any) => it.type === 'Object3D');
    list.forEach((it: any) => {
      if (it.update) it.update();
    });

    for (let i = 0; i < this.scene.children.length; i++) {
      let g = this.scene.children[i];
      if (g instanceof THREE.SpotLightHelper) {
        g.update();
      }
    }
  ...
  updateSize() {
    ...
    if (
      this.gl.drawingBufferWidth != size.x ||
