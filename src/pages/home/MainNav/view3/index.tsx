...
const LeftView = observer((props: any) => {
  ...
  useEffect(() => {
    // 默认添加 新疆市注记
    let xinJiang = pageDatas[0].childs[0];
    const _title = `${xinJiang.metaId}_${xinJiang.title}`;
    opMapLayers(xinJiang, _title, "add");
  ...
  return (
    ...
    >
          ...
          {pageDatas &&
            pageDatas.map((item: Data, index: number) => {
              ...
              return (
                ...
                >
                  <div
                    className={`${styles.list_li_bg} ${selMenu?.title == item.title ? styles.list_li_bg_sel : ""
                      }`}
                  ></div>
      ...
      {LeftMenu.getInstance().isShow() && <DataView opMapLayers={opMapLayers} />}
    </div>
  );
});
